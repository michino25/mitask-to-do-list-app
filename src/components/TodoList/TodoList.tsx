...
export default function TodoList() {
  ...
  return (
    <div className="relative flex justify-center items-center p-5 w-full h-full min-h-screen">
      <div className="fixed inset-0 -z-10" style={divStyle}></div>
      <div className="p-6 mb-8 bg-white/50 backdrop-blur-xl rounded-3xl">
        <div>
          <h5 className="text-3xl font-semibold text-gray-900">Work Tasks</h5>
        ...
        <div className="flex w-full">
          ...
          <button
            ...
            className={
              ...
              (input ? " bg-amber-500 hover:bg-amber-600" : " bg-amber-500/60")
            }
          >
            <svg
              className="mr-1"
        ...
        <div className={todos.length === 0 ? "hidden" : "flex justify-between"}>
          <span className="inline-flex items-center select-none tracking-wider px-3 py-2 text-sm font-medium text-center bg-amber-50 text-amber-600 rounded-xl border-1 border-amber-500">
            {todos.filter((item: Todo) => item.completed).length}/{todos.length}
          </span>
          <div>
            <button
              onClick={sortTasks}
      ...
      <div className="absolute flex items-center text-gray-600/60 bottom-3">
        <svg
        ...
        <span className="text-sm">Not suitable for mobile devices</span>
      </div>
    </div>
  );
}
